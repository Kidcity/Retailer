...
import { View, Text, ScrollView, FlatList, TouchableOpacity, Linking, BackHandler, Alert } from 'react-native';
import CustomHeader from '../../component/CustomHeader';
import { styles } from './style';
import { cart_map_pointer, images, setWidth } from '../../utils/variable';
import colors from '../../utils/colors';
import CartView from '../../component/CartView';
import CartAddressView from '../../component/CartAddressView';
import CartPaymentView from '../../component/CartPaymentView';
import CityWaletModal from '../../component/CityWaletModal';
...
import CartPageMapping from '../../component/CartPageMapping';
...
class CartScreen extends Component {
    ...
    setCartMapping(index) {
        const cart_map_pointer = this.state.cart_map_pointer

        if (index == 1) {
            cart_map_pointer[0].isDone = true
            cart_map_pointer[0].showRedDash = true
            this.setState({
                currentMapIndex: 1,
                cart_map_pointer: cart_map_pointer
            })
        } else if (index == 2) {
            cart_map_pointer[1].isDone = true
            cart_map_pointer[1].showRedDash = true
            this.setState({
                currentMapIndex: 2,
                cart_map_pointer: cart_map_pointer,
                page_heading: "SELECT ADDRESS"
            })
        } else {
            cart_map_pointer[2].isDone = true
            cart_map_pointer[2].showRedDash = true
            this.setState({
                currentMapIndex: 3,
                cart_map_pointer: cart_map_pointer,
                page_heading: "SELECT PAYMENT METHOD"
            })
        }
    }

    onpressFooterButton() {
        let currentMapIndex = this.state.currentMapIndex

        if (currentMapIndex != 3) {
            if (currentMapIndex == 2 && this.state.notDeliverable !== false) {
                errorAlert("Sorry!", "Order cannot be deliver in your address.\nPlease change your address.")
                return
            }
            currentMapIndex += 1
            this.setCartMapping(currentMapIndex)
        } else {
            this._showPaymentBreakUpModal()
        }
    }

    async checkMinimumOrderPrice(item_data) {
        if (+item_data.gross_total_price < +item_data.minimum_order_amount) {
            ...
            this.setState({
                ...
                emptyCartModalTitle: "ADD PRODUCT FOR RS (" + remain + ") TO REACH MINIMUM ORDER VALUE OF RS. " + item_data.minimum_order_amount
            })
    ...
    async onCityWiseBuy(item_data) {
        const result = await this.checkMinimumOrderPrice(item_data)
        if (result) {
            ...
            const selected_city_id = item_data.city_id
            const selected_city_name = item_data.city_name
            this.setState({
            ...
            // console.log(this.props.default_shipping_address);
            this.onpressFooterButton()
        }
    }
    ...
    async _rupifiCreateOrder(amount) {
       ...
       // console.log('_rupifiCreateOrder', param);
        // return
        let status = false
    ...
    resetStockState() {
        ...
        if (cart_items.length == 0) {
            ...
            const city_id = this.state.selected_city_id
            const findIndex = cart_items.findIndex((element) => element.city_id == city_id)
            if (findIndex != -1) {
    ...
    async _checkStockStatus() {
        ...
        const findIndex = cart_items.findIndex(element => element.city_id = this.state.selected_city_id)
    ...
    async _getShippingDetails(total) {
        ...
        await CartServices._getShippingCharges({ item_value: total, pincode: default_shipping_address.pin }).then(response => {
    ...
    async _updateCartItemsInLocalState(qty_ordered, parentIndex, childIndex) {
        ...
        let total_price_details = this.state.total_price_details
        let city_id = cart_items[parentIndex].city_id
        ...
        if (qty_ordered == 0) {
            ...
            let shipping_details = await this._getShippingDetails(total_order_price)
    ...
    render() {
        return (
            <View style={styles.container}>
                ...
                <ScrollView contentContainerStyle={styles.content}>
                    {
                        this.state.noCartData ?
                            ...
                            :
                            <>
                                <View style={styles.mapPointerView}>
                                    <CartPageMapping
                ...
                {
                    this.state.total_order_value != 0 &&
                    <View style={styles.footer}>
                        <View style={styles.row}>
                            <Text style={[styles.subHeading]}>Total </Text>
                            <Text style={[styles.subHeading, styles.textBold]}> ₹ {parseFloat(this.state.total_price_to_pay).toFixed(2)}</Text>
                        </View>
                ...
                }
                {
                    ...
                    <ThankYouModal
                        ...
                        onBackToHome={() => { this.setState({ showSuccessPaymentModal: false }); this.props.navigation.navigate('Home') }}
                        onGoToOrders={() => { this.setState({ showSuccessPaymentModal: false }); this.props.navigation.navigate("MyOrders") }}
                    />
                }
                {
